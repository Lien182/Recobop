...
THREAD_ENTRY() {
	...
	while (1) {
		...
		if (!strncmp(header, "GET / ", 6)) {
			...
			send(client_sock, "Content-Type: text/plain\n\n", 26, 0);
			buf_count = snprintf(buf, 1024, "%d", rbi_thread_count_m(rb_info, "touch", RECONOS_THREAD_SW));
			send(client_sock, buf, buf_count, 0);
		} else if (!strncmp(header, "GET /thread/touch/hw/count ", 27)) {
			send(client_sock, "Content-Type: text/plain\n\n", 26, 0);
			buf_count = snprintf(buf, 1024, "%d", rbi_thread_count_m(rb_info, "touch", RECONOS_THREAD_HW));
			send(client_sock, buf, buf_count, 0);
			...
			send(client_sock, "Content-Type: text/plain\n\n", 26, 0);
			buf_count = snprintf(buf, 1024, "%d", rbi_thread_count_m(rb_info, "control", RECONOS_THREAD_SW));
			send(client_sock, buf, buf_count, 0);
		} else if (!strncmp(header, "GET /thread/control/hw/count ", 29)) {
			send(client_sock, "Content-Type: text/plain\n\n", 26, 0);
			buf_count = snprintf(buf, 1024, "%d", rbi_thread_count_m(rb_info, "control", RECONOS_THREAD_HW));
			send(client_sock, buf, buf_count, 0);
			...
			send(client_sock, "Content-Type: text/plain\n\n", 26, 0);
			buf_count = snprintf(buf, 1024, "%d", rbi_thread_count_m(rb_info, "inverse", RECONOS_THREAD_SW));
			send(client_sock, buf, buf_count, 0);
		} else if (!strncmp(header, "GET /thread/inverse/hw/count ", 29)) {
			send(client_sock, "Content-Type: text/plain\n\n", 26, 0);
			buf_count = snprintf(buf, 1024, "%d", rbi_thread_count_m(rb_info, "inverse", RECONOS_THREAD_HW));
			send(client_sock, buf, buf_count, 0);
			...
			send(client_sock, "Content-Type: text/plain\n\n", 26, 0);
			buf_count = snprintf(buf, 1024, "%d", rbi_thread_count_m(rb_info, "servo", RECONOS_THREAD_SW));
			send(client_sock, buf, buf_count, 0);
		} else if  (!strncmp(header, "PUT /thread/inverse/sw/stop ", 28)) {
			int rti = rbi_thread_index(rb_info, "inverse", RECONOS_THREAD_SW);
			...
			rb_info->thread_p[rti] = reconos_thread_create_swt_inverse((void *)rb_info);
			send(client_sock, "\n", 1, 0);
		} else if (!strncmp(header, "GET /thread/servo/hw/count ", 27)) {
			send(client_sock, "Content-Type: text/plain\n\n", 26, 0);
			buf_count = snprintf(buf, 1024, "%d", rbi_thread_count_m(rb_info, "servo", RECONOS_THREAD_HW));
			send(client_sock, buf, buf_count, 0);
		} else if  (!strncmp(header, "PUT /thread/inverse/hw/stop ", 28)) {
			int rti = rbi_thread_index(rb_info, "inverse", RECONOS_THREAD_HW);
			reconos_thread_signal(rb_info->thread_p[rti]);
			reconos_thread_join(rb_info->thread_p[rti]);
			rb_info->thread_p[rti] = NULL;
			send(client_sock, "\n", 1, 0);
			...
			int rti = rbi_thread_index_free(rb_info);
			rb_info->thread_p[rti] = reconos_thread_create_hwt_inverse((void *)rb_info);
			send(client_sock, "\n", 1, 0);
